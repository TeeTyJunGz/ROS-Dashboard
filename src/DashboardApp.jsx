...
const getInitialState = (robotId) => {
  ...
  try {
    const stored = window.localStorage.getItem(`rosDashboardState-${robotId}`)
    if (stored) {
...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(`rosDashboardState-${robotId}`, JSON.stringify({ pages, currentPageId }))
    } catch (error) {
      console.error('Failed to persist dashboard state:', error)
    }
  }, [pages, currentPageId, robotId])
